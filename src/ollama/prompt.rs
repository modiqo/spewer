...
fn build_blocking(request: &TaskRequest, workspace: &Path) -> Result<String> {
    ...
    Ok(format!(
        "You are a bounded local inference worker. You have no tools and cannot modify files. Use only the supplied task and projected context. Return a direct answer without claiming that you ran commands or changed the workspace.\n\nObjective:\n{}\n\nAcceptance criteria:\n{}\n\nConstraints:\n{}\n\nBound skill instructions:\n{}\n\nProjected files:\n{}\n\n/no_think",
